## Web Framewrok

### Spring

- [Spring Dependency Injection](https://www.baeldung.com/spring-dependency-injection)
- [Spring AOP](https://www.baeldung.com/spring-aop)

### Spring Web MVC

- [Web on Servlet Stack](https://docs.spring.io/spring-framework/docs/current/reference/html/web.html)
- [Form Validation](https://spring.io/guides/gs/validating-form-input/)
- [Understanding Spring Web Application Architecture: The Classic Way](https://www.petrikainulainen.net/software-development/design/understanding-spring-web-application-architecture-the-classic-way/)

### Spring Boot

- [Spring Boot](https://docs.spring.io/spring-boot/docs/current/reference/htmlsingle/)

---

## Java Persistence API

### ORM framework

- [Hibernate ORM](https://hibernate.org/orm/documentation/5.4/)
- [Spring Data JPA](https://docs.spring.io/spring-data/jpa/docs/current/reference/html/#reference)

### Entity lifecycle

- [A beginner's guide to entity state transitions with JPA and Hibernate](https://vladmihalcea.com/a-beginners-guide-to-jpa-hibernate-entity-state-transitions/)
- [How do JPA persist, merge and Hibernate save, update, saveOrUpdate methods work](https://vladmihalcea.com/jpa-persist-merge-hibernate-save-update-saveorupdate/)
- [How do persist and merge work in JPA](https://vladmihalcea.com/jpa-persist-and-merge/)
- [A beginner's guide to flush strategies in JPA and Hibernate](https://vladmihalcea.com/a-beginners-guide-to-jpahibernate-flush-strategies/)

### Mapping (ORM)

- [Java & Databases: Guide to JDBC, Hibernate and Spring Data](https://www.marcobehler.com/guides/java-databases-jdbc-hibernate-spring-data)
- [Java persistence with JPA and Hibernate, Part 1: Entities and relationships](https://www.javaworld.com/article/3373652/java-persistence-with-jpa-and-hibernate-part-1-entities-and-relationships.html)
- [Java persistence with JPA and Hibernate, Part 2: Many-to-many relationships](https://www.javaworld.com/article/3387643/java-persistence-with-jpa-and-hibernate-part-2-many-to-many-relationships.html)
- [The best way to map a @OneToOne relationship with JPA and Hibernate](https://vladmihalcea.com/the-best-way-to-map-a-onetoone-relationship-with-jpa-and-hibernate/)
- [The best way to map a @OneToMany relationship with JPA and Hibernate](https://vladmihalcea.com/the-best-way-to-map-a-onetomany-association-with-jpa-and-hibernate/)
- [Access Strategies in JPA and Hibernate – Which is better, field or property access?](https://thorben-janssen.com/access-strategies-in-jpa-and-hibernate/)

### Pagination

- [Pagination with JPA and Hibernate](https://thorben-janssen.com/pagination-jpa-hibernate/)

### Primary keys

- [How to generate primary keys with JPA and Hibernate](https://thorben-janssen.com/jpa-generate-primary-keys/)

### Optimistic locking

- [Optimistic locking version property with JPA and Hibernate](https://vladmihalcea.com/optimistic-locking-version-property-jpa-hibernate/)

### Pitfalls

- [EAGER fetching is a code smell when using JPA and Hibernate](https://vladmihalcea.com/eager-fetching-is-a-code-smell/)
- [N+1 Problem](https://www.javacodegeeks.com/2018/04/jpa-tips-avoiding-the-n-1-select-problem.html)
- [N+1 query problem with JPA and Hibernate](https://vladmihalcea.com/n-plus-1-query-problem/)
- [How to avoid the Hibernate Query Cache N+1 issue](https://vladmihalcea.com/hibernate-query-cache-n-plus-1-issue/)
- [LazyInitializationException - What it is and the best way to fix it](https://thorben-janssen.com/lazyinitializationexception/)

### Performance tuning

- [Hibernate performance tuning tips](https://vladmihalcea.com/hibernate-performance-tuning-tips/)
- [The Best Way to Prevent Java Persistence and Hibernate Performance Issues](https://www.javaadvent.com/2019/12/the-best-way-to-prevent-java-persistence-and-hibernate-issues.html)

---

## RDBMS

### Tutorial

- [關聯式資料庫系統](http://database.klab.tw/teaching.php)
- [App 開發基礎講義 資料庫:基礎篇](https://ihower.tw/cs/database.html)
- [App 開發基礎講義 資料庫:進階篇](https://ihower.tw/cs/database-part2.html)
- [DBMS Tutorial](https://www.guru99.com/what-is-dbms.html)
- [Free SQL 101 Class Bootcamp Beginners Intro to SQL NYC](https://www.udemy.com/course/sql-analytics-coding-101-bootcamp-begineers-course-hand-on/)

### Transaction

- [A beginner's guide to ACID and database transactions](https://vladmihalcea.com/a-beginners-guide-to-acid-and-database-transactions/)
- [複習資料庫的 Isolation Level 與圖解五個常見的 Race Conditions](https://medium.com/@chester.yw.chu/%E8%A4%87%E7%BF%92%E8%B3%87%E6%96%99%E5%BA%AB%E7%9A%84-isolation-level-%E8%88%87%E5%B8%B8%E8%A6%8B%E7%9A%84%E4%BA%94%E5%80%8B-race-conditions-%E5%9C%96%E8%A7%A3-16e8d472a25c)
- [A beginner's guide to database locking and the lost update phenomena](https://vladmihalcea.com/a-beginners-guide-to-database-locking-and-the-lost-update-phenomena/)

### Index

- [PostgreSQL Indexes: First Principles](https://ieftimov.com/post/postgresql-indexes-first-principles/)
- [MySQL Indexing 101](https://medium.com/@stormanning/mysql-indexing-101-660f3193dde1)
- [Choosing the best indexes for MySQL query optimization](https://www.eversql.com/choosing-the-best-indexes-for-mysql-query-optimization/)
- [Indexing 101: Optimizing MySQL queries on a single table](https://www.percona.com/blog/2015/04/27/indexing-101-optimizing-mysql-queries-on-a-single-table/)

---

## MySQL

### Primary keys

- [为什么建议 MySQL InnonDB 的表使用递增的主键 ID](https://xlzd.me/2018/09/14/why-mysql-innodb-need-auto-increment-primary-key/)
- [淺談 InnoDB 的 Cluster Index 和 Secondary Index](https://medium.com/@genchilu/%E6%B7%BA%E8%AB%87-innodb-%E7%9A%84-cluster-index-%E5%92%8C-secondary-index-f75da308352e)

### Explain

- [MySQL EXPLAIN Statement](https://dev.mysql.com/doc/refman/8.0/en/explain.html)

---

## Redis

### Data types

- [Data types](https://redis.io/topics/data-types)
- [An introduction to Redis data types and abstractions](https://redis.io/topics/data-types-intro)

---

## REST Principles

- [REST API Guide](https://dev.to/drminnaar/rest-api-guide-14n2)
- [Best Practices for Designing a Pragmatic RESTful API](https://www.vinaysahni.com/best-practices-for-a-pragmatic-restful-api)
- [Traversing with Pagination](https://developer.github.com/v3/guides/traversing-with-pagination/)

## Design Pattern

- [Using Java Optional Correctly](https://www.dariawan.com/tutorials/java/using-java-optional-correctly/)
- [Builder Pattern](https://medium.com/@curryyen/%E8%81%8A-java-builder-pattern-660dd2cffea6)

## Testing

- [暸解單元測試](http://otischou.tw/2019/08/02/unit-test.html)
- [Unit Testing with Spring Boot](https://reflectoring.io/unit-testing-spring-boot/)
- [Testing Spring MVC Web Controllers with @WebMvcTest](https://reflectoring.io/spring-boot-web-controller-test/)
- [Testing JPA Queries with @DataJpaTest](https://reflectoring.io/spring-boot-data-jpa-test/)
- [Integration Tests with @SpringBootTest](https://reflectoring.io/spring-boot-test/)
- [Combining Object Mother and Fluent Builder for the Ultimate Test Data Factory](https://reflectoring.io/objectmother-fluent-builder/)
